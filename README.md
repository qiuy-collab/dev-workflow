...
### 3.1B Functional Supplements (Mandatory)
- After users state initial core features, embed “functional supplements” into the questioning in option-select format.
- The user must be able to choose or add missing functions; do not auto-assume.
- Summarize selections and confirm before proceeding.

### 3.1A UI Visualization and Confirmation (Mandatory)
...
### 3.4A No Fake Tests / No Fabricated Reports
- Reports and summaries must be produced from **real executions** only.
...
- If `logs/workflow.log` is missing, create it immediately and log an ERROR about missing logging.
...
### 3.10 Evidence Requirement for PASS
- Each PASS must include concrete evidence (command output, response, screenshot, or log reference).
- Evidence must come from the same run as the test point.
...
- UI quality checks are executed only in `final-delivery` and referenced in the final comprehensive report.
